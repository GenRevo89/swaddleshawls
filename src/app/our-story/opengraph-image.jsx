...
export const alt = 'SwaddleShawls - Our Heritage Story';
export const size = { width: 1200, height: 630 };
export const contentType = 'image/png';

export default async function Image() {
  const fontCss = await fetch(`https://fonts.googleapis.com/css2?family=Playfair+Display:wght@700&text=${encodeURIComponent('Our Story Heritage & Block Printing')}`, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/534.57.2 (KHTML, like Gecko) Version/5.1.7 Safari/534.57.2' } }).then((res) => res.text());
  ...
  const bgData = await fetch(new URL('../../../public/og/story_bg.png', import.meta.url)).then((res) => res.arrayBuffer());
